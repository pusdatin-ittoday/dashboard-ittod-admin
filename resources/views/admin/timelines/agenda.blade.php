    ...
    <div class="mb-6 overflow-hidden rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <div class="flex items-center gap-2">
                    ...
                    @elseif($regState === 'full')
                        <span class="rounded-full bg-purple-100 px-2.5 py-0.5 text-xs font-semibold text-purple-800">Pendaftaran Ditutup (Kuota Penuh)</span>
                ...
                <div class="mt-2 flex flex-wrap items-center gap-2 text-xs text-gray-600">
                    ...
                    @else
                        <span class="text-gray-500">Jadwal: Kontrol Manual</span>
                    @endif

                    @if($event->type === 'non_competition')
                        <span>•</span>
                        @if($event->max_noncompetition_participant)
                            <span>Kuota: <strong class="text-amber-800">{{ number_format($event->participants_count ?? 0, 0, ',', '.') }} / {{ number_format($event->max_noncompetition_participant, 0, ',', '.') }} peserta</strong></span>
                        @else
                            <span>Kuota: <strong class="text-gray-500">Tanpa batas</strong></span>
                        @endif
                    @endif
                    ...
                    @elseif($regState === 'full')
                        <span class="rounded bg-purple-50 border border-purple-200 px-2 py-0.5 text-[11px] font-bold text-purple-700">Tutup Otomatis (Kuota Maksimal Terpenuhi)</span>
                    @elseif($regState === 'closed')
            ...
            @if($canManageTimelines)
                ...
                >
                    {{ $event->type === 'non_competition' ? 'Atur Jadwal & Kuota' : 'Atur Timeline Pendaftaran' }}
                </button>
            @endif
        </div>
    </div>
    ...
    @if ($canManageTimelines)
        ...
        <x-modal name="edit-registration-deadline-{{ $event->id }}" maxWidth="lg" focusable>
            <form method="POST" action="{{ route('admin.timelines.registration-deadline', $event) }}" class="p-6">
                ...
                @method('PATCH')
                <div class="border-b border-gray-200 pb-4">
                    <h3 class="text-lg font-semibold text-gray-950">{{ $event->type === 'non_competition' ? 'Atur Jadwal & Kuota Pendaftaran' : 'Atur Timeline Jadwal Pendaftaran' }}</h3>
                    <p class="mt-1 text-sm text-gray-600">Pilih agenda timeline dan tentukan batasan kuota untuk otomatisasi pendaftaran {{ $event->title }}.</p>
                </div>
                ...
                <div class="mt-5 space-y-4">
                    ...
                    <div class="rounded-md bg-blue-50 p-3 text-xs text-blue-700 border border-blue-200">
                        <p class="font-semibold mb-1">Cara Kerja Otomatisasi Pendaftaran:</p>
                        <ul class="list-disc pl-4 space-y-0.5 text-blue-600">
                            ...
                            @if($event->type === 'non_competition')
                                <li>Jika pembatasan kuota dicentang, pendaftaran otomatis ditutup ketika pendaftar mencapai batas maksimal.</li>
                            @endif

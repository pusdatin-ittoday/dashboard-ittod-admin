<x-admin.layout
    title="Agenda Kompetisi Global"
    subtitle="Kelola daftar agenda yang berlaku serentak untuk semua kompetisi IT Today."
>
        ...
        <div class="flex flex-col gap-3 border-b border-gray-200 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <div class="flex flex-wrap items-center gap-3">
                    <h2 class="text-xl font-semibold text-gray-950">Direktori Agenda Kompetisi</h2>
                    <span class="rounded border border-emerald-200 bg-emerald-50 px-2 py-1 text-[10px] font-bold uppercase text-emerald-700">
                        Agenda Records
                    </span>
                </div>
            </div>
            <p class="text-xs font-semibold uppercase tracking-wide text-gray-500">{{ $competitionTimelines->count() }} records detected</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                    @forelse ($competitionTimelines as $agenda)
                        <tr
                            x-show="$el.dataset.search.includes(search.toLowerCase())"
                            data-search="{{ Str::lower($agenda->title . ' ' . ($agenda->start_date?->format('d M Y H:i') ?? '') . ' ' . ($agenda->end_date?->format('d M Y H:i') ?? '')) }}"
                        ...
                        >
                            ...
                            <td class="px-6 py-4 text-sm font-semibold text-gray-700">{{ \Carbon\Carbon::parse($agenda->start_date)->format('d M Y H:i') }}</td>
                            <td class="px-6 py-4 text-sm font-semibold text-gray-700">{{ \Carbon\Carbon::parse($agenda->end_date)->format('d M Y H:i') }}</td>
                            @if ($canManageTimelines)
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <form method="POST" action="{{ route('admin.timelines.destroy-global', $agenda) }}" onsubmit="return confirm('Hapus agenda ini?')">
                                            @csrf
                                            @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $canManageTimelines ? 3 : 2 }}" class="px-6 py-10 text-center text-sm text-gray-600">Belum ada agenda kompetisi global.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if ($canManageTimelines)
        <x-modal name="create-agenda" maxWidth="lg" focusable>
            <form method="POST" action="{{ route('admin.timelines.store-global') }}" class="p-6">
                @csrf

                <div class="border-b border-gray-200 pb-4">
                    <h3 class="text-lg font-semibold text-gray-950">Tambah Agenda</h3>
                    <p class="mt-1 text-sm text-gray-600">Kompetisi Global</p>
                </div>
        ...
        @foreach ($competitionTimelines as $agenda)
            <x-modal name="edit-agenda-{{ $agenda->id }}" maxWidth="lg" focusable>
                <form method="POST" action="{{ route('admin.timelines.update-global', $agenda) }}" class="p-6">
                    @csrf
                    @method('PATCH')
                    ...
                    <label class="block">
                        ...
                        <input
                            type="datetime-local"
                            name="start_date"
                            value="{{ old('start_date', \Carbon\Carbon::parse($agenda->start_date)->format('Y-m-d\TH:i')) }}"
                            required
                            class="mt-1 w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500"
                        >
                    </label>
                    ...
                    <label class="block">
                        ...
                        <input
                            type="datetime-local"
                            name="end_date"
                            value="{{ old('end_date', \Carbon\Carbon::parse($agenda->end_date)->format('Y-m-d\TH:i')) }}"
                            required
                            class="mt-1 w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500"
                        >
                    </label>
